...
import {AWSConfig, s3, cdnEndpoint, cdnAccessKeyId, cdnPrivateKeyString} from '../awsConfig';
...
class VideoUpload extends Component{
	...
	uploadVideo = (event) => {
		...
		reader.onload=(event)=>{this.setState({file:reader.result,filename:filename,filetype:filetype,complete:0});};
  ...
  //       });
	...
	changeValue(event){
    	...
    	this.setState({file:event.target.files[0],filename:event.target.files[0].name,filetype:event.target.files[0].type});
	}
	...
	render(){
		if(this.state.url){
			return (
				...
				<br/>
				<Progress animated value={this.state.complete}>{this.state.complete}%</Progress>
				<br/>
				<video id="clip" src={this.state.url} controls autoPlay width="640" height="264" />
				</div>
			);
		}
		else{
